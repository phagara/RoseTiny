...
#define SHORT_MIN 16   /* 444 microseconds */
#define SHORT_MAX 49  /* 1333 microseconds */
#define LONG_MIN 50   /* 1334 microseconds */
#define LONG_MAX 83   /* 2222 microseconds */
...
void RC5_Init()
{
    ...
    MCUCR |= _BV(ISC00);
    /* Set IR_PIN to input with pull-up */
    DDRB &= ~_BV(IR_PIN);
    PORTB |= _BV(IR_PIN);
    
    /* Enable Timer0 in normal mode with /256 clock prescaling */
    /* One tick is 26us with 9.6MHz clock */
    TCCR0B = _BV(CS02);
...
ISR(INT0_vect)
{
     ...
     *  6 - long pulse
     */
    uint8_t event = (PINB & _BV(IR_PIN)) ? 2 : 0;
